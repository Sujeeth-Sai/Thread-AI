...
Apply the files in `supabase/migrations/` in timestamp order to the configured Supabase project. The schema includes RLS policies that scope every research record to the authenticated project owner. The cleanup migration removes records from earlier development builds. `supabase/seed.sql` intentionally inserts nothing.
...
The build creates `apps/extension/thread-extension.zip` and copies the same archive to `public/thread-extension.zip`, which is the file served by every **Download extension** button on the website. Load `apps/extension/dist` through `chrome://extensions` → Developer mode → Load unpacked, or extract either generated ZIP and load that folder. In the popup:
